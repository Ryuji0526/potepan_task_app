...
describe 'tasks function', type: :system do
  ...
  let(:task_b) {FactoryBot.create(:task, title: '', start_date: Date.current.ago(3.days),end_date: Date.current.ago(5.days))}
  ...
  describe 'new function' do
    ...
    context 'form task with errors' do
      before do
        visit new_task_path
        fill_in "task[title]", with: task_b.title
        fill_in '開始日', with: task_b.start_date
        fill_in '終了日', with: task_b.end_date
        click_button 'スケジュールを登録する'
      end
