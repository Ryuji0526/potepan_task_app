  ...
  describe 'index view' do
    context 'when tasks exist' do
      ...
      it 'tasks displayed' do
        ...
        expect(page).to have_content date.strftime("%Y年%m月%d日")
  ...
  describe 'create function' do
    context 'form task without errors' do
      before do
        ...
        fill_in '開始日', with: Time.today
        fill_in '終了日', with: Time.today.since(3.days)
        click_button 'スケジュールを登録する'
      end

      it 'new task displayed' do
        expect(page).to have_content 'first test'
        expect(page).to have_content '2021年04月20日'
        expect(page).to have_content '2021年04月25日'
      end
    end
