...
describe 'tasks function', type: :system do
  ...
  describe 'delete function' do
    ...
    shared_examples_for 'task deleted' do
      it {
        within '.alert-success' do
          expect(page).to have_content "タスク「#{task_a.title}」を削除しました。"
        end
        expect(page).to have_no_content task_a.title
      }
    end

    context 'delete task from root_path' do
      before do
        visit root_path
        click_on '削除'
      end

      it_behaves_like 'task deleted'
    end

    context 'delete task from view_path' do
      before do
        visit task_path(task_a)
        click_on '削除'page.driver.browser.switch_to.alert.accept
      end
